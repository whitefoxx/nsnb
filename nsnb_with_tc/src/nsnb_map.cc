...
set<int> transform(string emailContent, TCMAP *featureSpace)
{
    ...
    for (i = 0; i <= len - NGRAM; i++)
    {
		...
		value = tcmapget2(featureSpace, feature.c_str());
        if (value)
        {
        ...
        else
        {
			...
			tcmapput2(featureSpace, feature.c_str(), value);
            dimension++;
        }
    }
